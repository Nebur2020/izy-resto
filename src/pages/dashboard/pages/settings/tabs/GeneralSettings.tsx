import { Building2, Shield } from 'lucide-react';
import { LogoUploader } from '../../../../../components/settings/LogoUploader';
import { useFormContext } from 'react-hook-form';
import { RestaurantSettings } from '../../../../../types';
import { SocialMediaSettings } from './SocialMediaSettings';
import { allCurrencies } from '../../../../../constants/defaultSettings';
import LanguageSwitcher from './LanguageSwitcher';
import { useTranslation } from 'react-i18next';

import PhoneInput, { CountryData } from 'react-phone-input-2';
import 'react-phone-input-2/lib/style.css';
import { useState } from 'react';

export function GeneralSettings() {
  const { t } = useTranslation();
  const {
    register,
    watch,
    setValue,
    formState: { errors },
  } = useFormContext<RestaurantSettings>();

  const [newCharacteristic, setNewCharacteristic] = useState('');

  const handleFieldChange = (
    field: 'logo' | 'coverImage' | 'language',
    value: string
  ) => {
    setValue(field, value, {
      shouldDirty: true,
      shouldTouch: true,
    });
  };

  const currency = watch('currency');
  const currencyObject = allCurrencies.find(cur => cur.value === currency);

  return (
    <div className="space-y-8">
      <section className="space-y-6">
        <div className="flex items-center gap-3 mb-6">
          <Building2 className="h-5 w-5 text-blue-600 dark:text-blue-400" />
          <h2 className="text-xl font-semibold">
            {t('setting:general-information')}
          </h2>
        </div>

        <div className="space-y-6">
          <div>
            <label className="block text-sm font-medium mb-1">
              {t('setting:restaurant-name')}
            </label>
            <input
              type="text"
              {...register('name', {
                required: t('setting:restaurant-name-required'),
              })}
              className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
                errors.name
                  ? 'border-red-500 dark:border-red-500'
                  : 'dark:border-gray-600'
              }`}
            />
            {errors.name && (
              <p className="mt-1 text-sm text-red-500">{errors.name.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">
              {t('setting:restaurant-description')}
            </label>
            <textarea
              {...register('description', {
                required: t('setting:restaurant-description-required'),
                maxLength: {
                  value: 250,
                  message: t('description-max-limit', {
                    maxLimit: 250,
                  }),
                },
                minLength: {
                  value: 10,
                  message: t('description-min-limit', { minLimit: 10 }),
                },
              })}
              rows={3}
              className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
                errors.description
                  ? 'border-red-500 dark:border-red-500'
                  : 'dark:border-gray-600'
              }`}
            />
            {errors.description && (
              <p className="mt-1 text-sm text-red-500">
                {errors.description.message}
              </p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">
              {t('common:email')}
            </label>
            <input
              type="email"
              {...register('email', {
                pattern: {
                  value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i,
                  message: t('common:email-invalid'),
                },
              })}
              className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
                errors.email
                  ? 'border-red-500 dark:border-red-500'
                  : 'dark:border-gray-600'
              }`}
            />
            {errors.email && (
              <p className="mt-1 text-sm text-red-500">
                {errors.email.message}
              </p>
            )}
          </div>

          <div>
            <LogoUploader
              value={watch('logo')}
              onChange={url => handleFieldChange('logo', url)}
            />
          </div>

          <div>
            <LogoUploader
              value={watch('coverImage')}
              onChange={url => handleFieldChange('coverImage', url)}
              label={t('setting:cover-image')}
              description={t('setting:image-recommanded-format')}
            />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">
              {t('setting:currency')}
            </label>
            <select
              {...register('currency', {
                required: t('setting:currency-required'),
              })}
              className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
                errors.currency
                  ? 'border-red-500 dark:border-red-500'
                  : 'dark:border-gray-600'
              }`}
            >
              {allCurrencies.map(currency => (
                <option key={currency.value} value={currency.value}>
                  {currency.label}
                </option>
              ))}
            </select>
            {errors.currency && (
              <p className="mt-1 text-sm text-red-500">
                {errors.currency.message}
              </p>
            )}

            <div
              className="flex mt-4 items-center p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800"
              role="alert"
            >
              <svg
                className="flex-shrink-0 inline w-4 h-4 me-3"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
              </svg>
              {currencyObject &&
                (currencyObject?.acceptedPaymentMethods?.length < 1 ? (
                  <span>
                    {t('setting:available-payment-method')}{' '}
                    <span className="font-semibold">
                      {t('setting:delivery-and-add-payment-method')}
                    </span>
                  </span>
                ) : (
                  <span>
                    {t('setting:available-payment-method')}{' '}
                    <span className="font-semibold">
                      {currencyObject?.acceptedPaymentMethods.join(', ')}{' '}
                    </span>
                  </span>
                ))}
            </div>
          </div>

          <LanguageSwitcher
            value={watch('language')}
            onLanguageChanged={language =>
              handleFieldChange('language', language)
            }
          />

          <div>
            <label className="block text-sm font-medium mb-1">
              {t('common:phone-number')} skjd
            </label>

            <PhoneInput
              country={watch('country.countryCode')}
              value={`+${watch('country.dialCode')}`}
              countryCodeEditable={false}
              containerClass="w-[100px] flex items-center border-r  !border-none"
              inputClass="!w-full !border-none bg-transparent pl-2 text-sm text-gray-700 !dark:text-gray-300 dark:bg-gray-800"
              buttonClass="!bg-transparent !dark:bg-transparent !border-none p-0 flex items-center"
              dropdownClass="absolute top-full z-50 bg-white dark:bg-gray-800 shadow-lg border  !border-none"
              searchClass="!border-none"
              onChange={(_, country: CountryData) => {
                setValue(
                  'country',
                  {
                    countryCode: country.countryCode,
                    dialCode: country.dialCode,
                    name: country.name,
                  },
                  {
                    shouldDirty: true,
                    shouldTouch: true,
                  }
                );
              }}
              enableSearch
            />
          </div>
        </div>
      </section>

      <section className="space-y-6">
        <div className="flex items-center gap-3">
          <Shield className="h-5 w-5 text-blue-600 dark:text-blue-400" />
          <h2 className="text-xl font-semibold">
            {t('setting:security-and-rate-limits')}
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium mb-1">
              {t('setting:rate-limit-max-orders')}
            </label>
            <input
              type="number"
              {...register('rateLimits.maxOrders', {
                required: t('setting:rate-limit-max-orders-required'),
                min: {
                  value: 1,
                  message: t('setting:rate-limit-max-orders-min', {
                    minValue: 1,
                  }),
                },
              })}
              className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
                errors.rateLimits?.maxOrders
                  ? 'border-red-500 dark:border-red-500'
                  : 'dark:border-gray-600'
              }`}
            />
            {errors.rateLimits?.maxOrders && (
              <p className="mt-1 text-sm text-red-500">
                {errors.rateLimits.maxOrders.message}
              </p>
            )}
            <p className="mt-1 text-sm text-gray-500">
              {t('setting:rate-limit-max-orders-description')}
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">
              {t('setting:rate-limit-time-window-hours')}
            </label>
            <input
              type="number"
              {...register('rateLimits.timeWindowHours', {
                required: t('setting:rate-limit-max-orders-required'),
                min: {
                  value: 1,
                  message: t('setting:rate-limit-time-window-hours-min', {
                    minValue: 1,
                  }),
                },
              })}
              className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
                errors.rateLimits?.timeWindowHours
                  ? 'border-red-500 dark:border-red-500'
                  : 'dark:border-gray-600'
              }`}
            />
            {errors.rateLimits?.timeWindowHours && (
              <p className="mt-1 text-sm text-red-500">
                {errors.rateLimits.timeWindowHours.message}
              </p>
            )}
            <p className="mt-1 text-sm text-gray-500">
              {t('setting:rate-limit-time-window-hours-description')}
            </p>
          </div>
        </div>
      </section>

      <SocialMediaSettings />

      <div>
        <h1 className="my-4">{t('common:theme-configuration')}</h1>

        <label className="block text-sm font-medium mb-1">
          {t('pizzatheme:restaurant-characteristics')}
        </label>

        <div className="flex items-center gap-2 mb-4">
          <input
            type="text"
            value={newCharacteristic}
            onChange={e => setNewCharacteristic(e.target.value)}
            className="w-full rounded-lg border p-2 dark:bg-gray-700 dark:border-gray-600"
            placeholder={t('pizzatheme:add-characteristics')}
          />
          <button
            type="button"
            onClick={() => {
              if (newCharacteristic.trim() !== '') {
                const currentCharacteristics =
                  watch('restaurantCharacteristics') || [];
                setValue(
                  'restaurantCharacteristics',
                  [...currentCharacteristics, newCharacteristic.trim()],
                  {
                    shouldDirty: true,
                    shouldTouch: true,
                  }
                );
                setNewCharacteristic('');
              }
            }}
            className="bg-blue-400 text-white px-4 py-2 rounded-lg"
          >
            {t('common:add')}
          </button>
        </div>
        <div className="space-y-2">
          {(watch('restaurantCharacteristics') || []).map(
            (characteristic, index) => (
              <div key={index} className="flex items-center gap-2">
                <span className="p-2 bg-gray-100 dark:bg-gray-700 rounded-lg">
                  {characteristic}
                </span>
                <button
                  type="button"
                  onClick={() => {
                    const updatedCharacteristics = (
                      watch('restaurantCharacteristics') || []
                    ).filter((_, i) => i !== index);
                    setValue(
                      'restaurantCharacteristics',
                      updatedCharacteristics,
                      {
                        shouldDirty: true,
                        shouldTouch: true,
                      }
                    );
                  }}
                  className="text-red-500"
                >
                  {t('common:remove')}
                </button>
              </div>
            )
          )}
        </div>

        <div className="mt-4">
          <label className="block text-sm font-bold mb-1">
            {t('pizzatheme:free-home-delivery-title')}
          </label>
          <input
            type="text"
            {...register('freeHomeDelivery')}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.name
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.name && (
            <p className="mt-1 text-sm text-red-500">{errors.name.message}</p>
          )}
        </div>

        <div className="mt-4">
          <label className="block text-sm font-bold mb-1">
            {t('pizzatheme:product-list-sub-title')}
          </label>
          <input
            type="text"
            {...register('productListTitle')}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.productListTitle
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.productListTitle && (
            <p className="mt-1 text-sm text-red-500">
              {errors.productListTitle.message}
            </p>
          )}
        </div>

        <div className="mt-4">
          <label className="block text-sm font-bold mb-1">
            {t('pizzatheme:product-list-description')}
          </label>
          <textarea
            {...register('productListDescription', {
              maxLength: {
                value: 250,
                message: t('description-max-limit', {
                  maxLimit: 250,
                }),
              },
              minLength: {
                value: 10,
                message: t('description-min-limit', { minLimit: 10 }),
              },
            })}
            rows={3}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.productListDescription
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.productListDescription && (
            <p className="mt-1 text-sm text-red-500">
              {errors.productListDescription.message}
            </p>
          )}
        </div>

        <div className="mt-4">
          <label className="block text-sm font-bold mb-1">
            {t('pizzatheme:restaurant-qualification-title')}
          </label>
          <textarea
            {...register('footerBannerTitle', {
              maxLength: {
                value: 250,
                message: t('description-max-limit', {
                  maxLimit: 250,
                }),
              },
              minLength: {
                value: 10,
                message: t('description-min-limit', { minLimit: 10 }),
              },
            })}
            rows={3}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.footerBannerTitle
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.footerBannerTitle && (
            <p className="mt-1 text-sm text-red-500">
              {errors.footerBannerTitle.message}
            </p>
          )}
        </div>

        <div className="mt-4">
          <label className="block text-sm font-medium mb-1">
            {t('pizzatheme:discount-rate')}
          </label>
          <input
            type="number"
            {...register('productDiscount', {
              min: {
                value: 1,
                message: t('setting:rate-limit-max-orders-min', {
                  minValue: 1,
                }),
              },
            })}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.productDiscount
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.productDiscount && (
            <p className="mt-1 text-sm text-red-500">
              {errors.productDiscount.message}
            </p>
          )}
          <p className="mt-1 text-sm text-gray-500">
            {t('setting:rate-limit-max-orders-description')}
          </p>
        </div>

        <div className="mt-4">
          <label className="block text-sm font-bold mb-1">
            {t('pizzatheme:free-delivery-title')}
          </label>
          <input
            type="text"
            {...register('freeDeliveryTitle')}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.freeDeliveryTitle
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.freeDeliveryTitle && (
            <p className="mt-1 text-sm text-red-500">
              {errors.freeDeliveryTitle.message}
            </p>
          )}
        </div>

        <div className="mt-4">
          <label className="block text-sm font-bold mb-1">
            {t('pizzatheme:free-delivery-description')}
          </label>
          <textarea
            {...register('freeDeliveryDescription', {
              maxLength: {
                value: 250,
                message: t('description-max-limit', {
                  maxLimit: 250,
                }),
              },
              minLength: {
                value: 10,
                message: t('description-min-limit', { minLimit: 10 }),
              },
            })}
            rows={3}
            className={`w-full rounded-lg border p-2 dark:bg-gray-700 ${
              errors.freeDeliveryDescription
                ? 'border-red-500 dark:border-red-500'
                : 'dark:border-gray-600'
            }`}
          />
          {errors.freeDeliveryDescription && (
            <p className="mt-1 text-sm text-red-500">
              {errors.freeDeliveryDescription.message}
            </p>
          )}
        </div>
      </div>
    </div>
  );
}
