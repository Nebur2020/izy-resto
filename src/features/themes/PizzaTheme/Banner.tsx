...
export default function Banner({
  ...
  return (
    <section
      className={`flex flex-col-reverse lg:flex-row justify-between items-center h-auto lg:h-screen px-6 lg:px-20 py-10 pt-20 ${
        isDarkModeActive ? 'bg-gray-900 text-white' : ''
      }`}
      style={{
        backgroundColor: isDarkModeActive
          ? '#1a1a1a'
          : themeConfig.general.backgroundColor,
      }}
    >
      <div className="w-full lg:w-[40%] text-center lg:text-left lg:ml-10">
        ...
        <ul className="py-5 space-y-2">
          {bannerTaglines.map((tagline, index) => (
            ...
            >
              <CheckCircle color={themeConfig.general.primaryColor} />
              <span
